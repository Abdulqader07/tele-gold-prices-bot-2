    ...
    async def checkPriceChange(self):
        ...
        if max_diff >= min_diff:
            if max_diff >= config.DIFF_THRESHOLD:
                ...
                await self.sendNotification(difference=max_diff, direction="DOWN", current_price=max_price, reference_price=current_price)
            ...
            if min_diff >= config.DIFF_THRESHOLD:
                ...
                await self.sendNotification(difference=min_diff, direction="UP", current_price=min_price, reference_price=current_price)
